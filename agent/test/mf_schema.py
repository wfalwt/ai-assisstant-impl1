...
class ObjectField(BaseModel):
    ...
    field_type: int = Field(description="""对象的字段类型，请选择枚举类型中的一种，数字代表的类型如下：
                            1. 文本
                            2. 多行文本
                            3. 数字
                            4. 日期
...
class FaasObject(BaseModel):
    ...
    action_type: int = Field(description="对象的操作类型 : 1. 创建 2. 更新 3. 删除 4. 高级更新",
                                        examples=[1])
    ...
    faas_condition_type: int = Field(
        description="函数查询字段间的关联关系，如并且、或者，其中 0. 无关系 1. 并且 2. 或者", default=None, examples=[1])


class AppData(BaseModel):
    object_list: list[Object] = Field(description="业务对象列表")
    faas_list: list[FaasObject] = Field(description="流程函数列表")


if __name__ == "__main__":
    print(AppData.model_json_schema())